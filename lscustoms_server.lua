...
emP = {}
Tunnel.bindInterface("vrp_lscolor",emP)

function emP.checkPermission()
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
		return vRP.hasPermission(user_id,"bennys.permissao") or vRP.hasPermission(user_id,"admin.permissao")
	end
end
...
function emP.AmamosOAga()
	local source = source
	local vehicle,vnetid,placa,vname,lock,banned = vRPclient.vehList(source,4)
	local valorCarro = vRP.vehiclePrice(vname)
	local desconto = 1

	return desconto
end

RegisterServerEvent("LSCOLOR:buttonSelected")
AddEventHandler("LSCOLOR:buttonSelected", function(name,button)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
		if button.price then
			if vRP.tryFullPayment(user_id,button.price) then
				TriggerClientEvent("LSCOLOR:buttonSelected",source,name,button,true)
			else
				TriggerClientEvent("LSCOLOR:buttonSelected",source,name,button,false)
			end
		else
			TriggerClientEvent("LSCOLOR:buttonSelected",source,name,button,false)
		end
...
RegisterServerEvent("LSCOLOR:finished")
AddEventHandler("LSCOLOR:finished",function(veh)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
		...
		if vehicle and placa then
			...
			if puser_id then
				...
				print(puser_id,vname,json.encode(custom))
